...
namespace ams::mitm::nifm {
    ...
    void InitializeDebug(bool enabled) {
        ...
        if (enabled) {
            ...
            mitm::fs::DeleteAtmosphereSdFile("/logs/nifm_mitm.log");
            R_ABORT_UNLESS(mitm::fs::CreateAtmosphereSdFile("/logs/nifm_mitm.log", 0, ams::fs::CreateOption_None));
        }
    }
    ...
    void LogDebug(const char *fmt, ...) {
        ...
        ::FsFile f;
        if (R_FAILED(mitm::fs::OpenAtmosphereSdFile(std::addressof(f), "/logs/nifm_mitm.log", ams::fs::OpenMode_ReadWrite | ams::fs::OpenMode_AllowAppend))) {
            return;
        }
