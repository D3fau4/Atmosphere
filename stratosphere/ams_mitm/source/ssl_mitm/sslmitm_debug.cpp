...
namespace ams::mitm::ssl {
    ...
    void InitializeDebug(bool enabled) {
        ...
        if (enabled) {
            ...
            mitm::fs::DeleteAtmosphereSdFile("/logs/ssl_mitm.log");
            R_ABORT_UNLESS(mitm::fs::CreateAtmosphereSdFile("/logs/ssl_mitm.log", 0, ams::fs::CreateOption_None));
        }
    }
    ...
    void LogDebug(const char *fmt, ...) {
        ...
        ::FsFile f;
        if (R_FAILED(mitm::fs::OpenAtmosphereSdFile(std::addressof(f), "/logs/ssl_mitm.log", ams::fs::OpenMode_ReadWrite | ams::fs::OpenMode_AllowAppend))) {
            return;
        }
